...
export const KNOWN_MAINNET_ORACLE_DEFAULTS = new Set([
...
export const hasVerifiedTestnetOracleConfig = (): boolean => {
...
/**
 * Decides whether the YEI oracle polling provider should be disabled for this runtime.
 *
 * Precedence (highest → lowest):
 *  1. DISABLE_YEI_ORACLE_PROVIDER=true  → always disable
 *  2. Non-testnet network               → always enable
 *  3. Testnet + no explicit opt-in      → disable
 *  4. Testnet + opt-in but bad addrs    → disable
 *  5. Testnet + opt-in + valid addrs    → enable
 */
export const getOracleProviderDecision = (): {
  disable: boolean;
  reason: string;
} => {
  ...
  const isTestnet = (process.env.SEI_NETWORK || "")
    .toLowerCase()
    .includes("testnet");
  if (!isTestnet) {
  ...
  if (!hasVerifiedTestnetOracleConfig()) {
    return {
      disable: true,
      reason:
        "testnet oracle addresses are missing/invalid or still using known defaults",
    };
  }
  ...
  return {
    disable: false,
    reason:
      "testnet oracle provider explicitly enabled with verified addresses",
  };
};
